...
ggswim offers several layering functions that mimic other "geom" functions from core ggplot2: `geom_swim_lane()`, `geom_swim_point()`, and `geom_swim_label()`. The former, an extension of `geom_segment()`, allows users to construct the horizontal bars, what we'll sometimes refer to as "lanes." Meanwhile, `geom_swim_point()` and `geom_swim_label` wrap both `geom_point()` and `geom_label()`, respectively, to effortlessly embed key events, or "markers," onto the lanes. These can take the form of shapes, symbols, or even emojis.
...
Because `new_scale_color()` establishes the start of a new scale, it is important to define any scale changes we want before calling it and moving on to the next one. It is a common workflow to use `scale_color_manual()`, `scale_color_brewer()`, `scale_color_discrete()`, etc., to change and update the colors and names of the visible elements in both the plot and the legend, so we'll apply it here, just after our swim lane layer but before moving on to adding the markers.
