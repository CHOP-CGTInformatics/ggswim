...
Wow! ... I think? It's pretty busy and a little hard to follow. We will discuss more about why this is in an upcoming vignette, but for now we provide you with an additional function to sort this all out: `fix_legend()`. `fix_legend()` must be used on a rendered ggplot object, it will not work with the `+` operator.

```{r}
fix_legend(p)
```

And now you can see that all of the emojis and shapes are correctly assigned in the legend! Now, about those colors and this ugly display....

## Making the Plot Pretty

Recall earlier that we said ggswim makes use of existing ggplot2 architecture. This means you can use your favorite manipulators to make your ggswim objects look however you please. Below, we take the pre-existing plot, `p`, and supply some beautifucation steps before sending it to `fix_legend()`:
